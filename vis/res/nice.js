...
class Graph {
	...
	constructor(container){
        ...
        this.CONSERVATION = 0.95;

        this.CORRECTION = 100;
        this.RENDERLINKS = false;
		this.link_deactivation = true;
		this.SIMULATE = true;

        this.nodeRadius = 2;
        this.sqrNodeRadius = this.nodeRadius * 2 * this.nodeRadius * 2;
        this.visualNodeScalingFactor = 1;
		this.linkWidth = 1;
    ...
    data(d){
        ...
        this.P = []; // node positions        (N, 2)
        this.D = []; // previous positions    (N, 2)
        this.R = []; // repulsion matrix      (N, N)
        this.T = []; // link transforms       (L, 3) 
        this.C = []; // link connections      (L, 2) 
        ...
        this.links.forEach(link => {
            link.width = graph.linkWidth;
            link.rect = graph.rect(link.width, "rgb(200,200,200)", 0.1);
            graph.R[link.src][link.dst] = -1;
            graph.R[link.dst][link.src] = -1;
        });

        this.nodes.forEach(node => {
			node.circle = graph.circle(graph.nodeRadius * graph.visualNodeScalingFactor, "rgb(200,200,200)", 1);
			if(!graph.clusters.includes(node.cluster)){
				graph.clusters.push(node.cluster);
			}
		});
        ...
        this.culombe = this.gpu.createKernel(function (p, r, lf) {

            let x = this.thread.y; // foreach node       [0, ..., SIZE]
            let y = this.thread.x; // foreach dimension  [0, 1]
            ...
            for (let i = 0; i < size; i++) {
                ...
                if(diff != 0){
                    ...
                    odiff = p[x][1-y] - p[i][1-y];
                    if(odiff != 0){
						sqdist = diff * diff + odiff * odiff;
						
						if(sqdist < 500*500){
							if(r[x][i] < 0){
								force -= diff * r[x][i] / sqdist * 0.1; // repulse
							}else{
								force += diff * r[x][i] / sqdist * 1; // repulse
							}
                        ...
                        if(r[x][i] < 0){
							...
							force += diff * r[x][i] * sqdist  / 100000 ; // attract
            ...
            vit = (vit * conservation + force * (1 - conservation));
			...
			if(vit * vit >= correction * correction){
        ...
        { constants: { cx: this.center.x, cy: this.center.y, correction: this.CORRECTION, size: this.N, conservation: this.CONSERVATION, sqrMinNodeDist:this.sqrNodeRadius } }).setOutput([2, this.N]).setOutputToTexture(true);


        this.linkTransform = this.gpu.createKernel(function (c, p) {
            let x = this.thread.y; // foreach link       [0, ..., SIZE]
            let y = this.thread.x; // foreach dimension  [0, 1]
            let dx = p[c[x][1]][0] - p[c[x][0]][0];
            let dy = p[c[x][1]][1] - p[c[x][0]][1];
            if(y == 0){
                return Math.sqrt(dx*dx + dy*dy);
            }else{
                let a = (Math.acos(dx / Math.sqrt(dx*dx + dy*dy)) / 3.14159 * 180);
                if(dy<0){
                    return -a;
                }
                else{
        ...
        { constants: {} }).setOutput([2, this.L]).setOutputToTexture(true);
    ...
    draw(l){
        for (let i = 0; i < this.N; i++) {
            this.nodes[i].circle.setAttribute("cx", this.P[i][0]);
            this.nodes[i].circle.setAttribute("cy", this.P[i][1]);
		}
		if(l){
            this.T = this.linkTransform(this.C,this.P).toArray(this.gpu);
            for (let i = 0; i < this.L; i++) {
                var link = this.links[i];
                link.rect.setAttribute("x", this.P[link.src][0]);
                link.rect.setAttribute("y", this.P[link.src][1] - this.linkWidth/2);
                link.rect.setAttribute("width", this.T[i][0]);
                link.rect.setAttribute("transform", "rotate("+ this.T[i][1] +", "+ this.P[link.src][0] +", "+ this.P[link.src][1] +")");
            }
            this.link_deactivation = true;
        }else{
            if(this.link_deactivation){
                this.link_deactivation = false;
                for (let i = 0; i < this.L; i++) {
                    var link = this.links[i];
                    link.rect.setAttribute("x", -1);
                    link.rect.setAttribute("y", -1);
                    link.rect.setAttribute("width", 0);
                }
            }
		}
    }
    ...
    cycle(){
        var tmp = this.P;
        this.P = this.culombe(this.P, this.R, this.D).toArray(this.gpu);
        this.D = tmp;
    }
	...
	cluster_colors(){
		...
		this.nodes.forEach(node => {
			if(sizes[node.cluster] == undefined){
				sizes[node.cluster] = 0;
			}
			sizes[node.cluster] += 1;
		});
		...
		for(var k in sizes){
			if(colors[k] == undefined){
				colors[k] = "rgb("+ Math.floor(Math.random() * 256) +", "+ Math.floor(Math.random() * 256) +", "+ Math.floor(Math.random() * 256) +")";
			}
		}
		...
		this.nodes.forEach(node => {
			node.circle.setAttribute("fill", colors[sizes[node.cluster]]);
		});

		this.links.forEach(link => {

			if(graph.nodes[link.src].cluster == graph.nodes[link.dst].cluster){
				link.rect.setAttribute("fill", colors[sizes[graph.nodes[link.src].cluster]]);
			}
    ...
    random_P(max){
        ...
        this.P = [];
        for (let i = 0; i < this.N; i++) {
            this.P.push([Math.random() * (max) + (this.center.x-max/2), Math.random() * (max) + (this.center.y-max/2)])
        }
        this.D = this.P;
    }
    ...
    init_C(){
        var graph = this;
        this.links.forEach(link => {
            graph.C.push([link.src, link.dst]);
        });
	}
...
class Display {
	...
	initSvg(){
		this.svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
		this.width = this.displayDiv.offsetWidth;
		this.height = this.displayDiv.offsetHeight;
		this.svg.setAttribute("width", this.width);
		this.svg.setAttribute("height", this.height);
		this.displayDiv.appendChild(this.svg);
		this.center = {"x": this.width / 2.0, "y": this.height / 2.0};
	...
	network(data){
		...
		this.clusterBackground = document.createElementNS("http://www.w3.org/2000/svg", "g");
		this.svg.appendChild(this.clusterBackground);
		...
		//---------------------------------------------------------------- setup data bounds from nodes
		this.nodes.forEach(node => {
			if(node.x < display.bounds.left){
				display.bounds.left = node.x;
			}
			if(node.x > display.bounds.right){
				display.bounds.right = node.x;
			}
			if(node.y > display.bounds.bottom){
				display.bounds.bottom = node.y;
			}
			if(node.y < display.bounds.top){
				display.bounds.top = node.y;
			}
		...
		//---------------------------------------------------------------- replace nodes according to bounds
		this.replaceNodes();
		...
		var ang;
		this.links.forEach(link => {
			a = display.nodes[link.dst];
			b = display.nodes[link.src];
			dx = a.wx - b.wx;
			dy = a.wy - b.wy;
			mag = Math.sqrt(dx * dx + dy * dy);

			link.rect = display.rect(mag, 1, a.wx, a.wy - 0.5, "#aaaaaa", 0.05);

			let ang = (Math.acos(dx / mag) / 3.14159 * 180) + 180;
            if(dy<0){
                ang = -ang;
            }
			link.rect.setAttribute("transform", "rotate("+ ang +", "+ a.wx +", "+ a.wy +")");
		...
		display.clusters = {};
		this.nodes.forEach(node => {
			if(display.clusters[node.cluster] == undefined){
		...
		var pt = display.svg.createSVGPoint();
		var clusterHover;
		display.clusterClick;

		function cursorPoint(evt){
			pt.x = evt.clientX; pt.y = evt.clientY;
			return pt.matrixTransform(display.svg.getScreenCTM().inverse());
		}
		...
		document.body.onresize = function(){
			display.redimensionDivs();
			display.replaceNodes();
			display.redrawNodes();
			display.redrawLinks();
		};
	...
	replaceNodes(){
		...
		display.width = this.displayDiv.offsetWidth;
		display.height = this.displayDiv.offsetHeight;
		display.svg.setAttribute("width", this.width);
		display.svg.setAttribute("height", this.height);

		var minl = Math.min(display.width, display.height);
	...
	highlightCluster(){
		var display = this;
		if(display.clusterClick != undefined){
			display.clusters[display.clusterClick].forEach(node => {
				var circ = document.createElementNS("http://www.w3.org/2000/svg", "circle");
				circ.setAttributeNS(null, "cx", node.wx);
				circ.setAttributeNS(null, "cy", node.wy);
				circ.setAttributeNS(null, "r", (node.radius + 8) * display.scalefactor);
	...
	redrawLinks(){
		...
		this.links.forEach(link => {
			...
			link.rect.setAttribute("width", mag);
			link.rect.setAttribute("x", a.wx);
			link.rect.setAttribute("y", a.wy - 0.5);
	...
	sizes(){
		this.nodes.forEach(node => {
			...
			node.circle.setAttribute("r", node.size * 0.9);
		});
	...
	cluster_colors(){
		...
		for(var k in sizes){
			if(colors[sizes[k]] == undefined){
				colors[sizes[k]] = "rgb("+ (100 + Math.floor(Math.random() * 151)) +", "+ (100 + Math.floor(Math.random() * 151)) +", "+ (100 + Math.floor(Math.random() * 151)) +")";
			}
		}
		...
		this.nodes.forEach(node => {
			...
			node.color = colors[sizes[node.cluster]];
		});

		this.links.forEach(link => {

			if(display.nodes[link.src].cluster == display.nodes[link.dst].cluster){
				link.rect.setAttribute("fill", colors[sizes[display.nodes[link.src].cluster]]);
			}
